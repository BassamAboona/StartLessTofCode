...
void StVpdCalibMaker::vzVpdFinder()
{
  for(int i=0;i<2*NVPD;i++){
    ...
    if(i<NVPD&&mNWest>1) {  // west VPD
      ...
      if(fabs(vpdtime)>TDIFFCUT && mCutVpdOutliers) { // bassam: note the addition of && mCutVpdOutliers in the if statement logic.
        mTSumWest -= mVPDLeTime[i];
        mVPDLeTime[i] = 0.;
        mNWest--;
  ...
  if(mTruncation && mCutVpdOutliers) { // bassam: note the addition of && mCutVpdOutliers in the if statement logic.
