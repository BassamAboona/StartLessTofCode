...
Int_t StBFChain::Instantiate()
{
  ...
  for (i = 1; i< fNoChainOptions; i++) {// Instantiate Makers if any
    ...
    if (maker == "StBTofCalibMaker" && GetOption("pppAMode")) mk->SetAttr("pppAMode", kTRUE); // bassam
    if (maker == "StBTofCalibMaker" && GetOption("setPPPAOutlierRej")) mk->SetAttr("setPPPAOutlierRej", kTRUE); // bassam
    if (maker == "StVpdCalibMaker" && GetOption("vpdPPPAMode")) mk->SetAttr("vpdPPPAMode", kTRUE); // bassam
